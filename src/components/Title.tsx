...
export const Title: FC<TitleType> = ({ title1, title2, text }) => {
  const { error } = colors;
  const { h2, h6 } = typography;

  return (
    <Box justifyItems='cen'>
      <Typography fontSize={h2} textTransform="uppercase">
        <span style={{ color: error.focus }}>{title1}</span> {title2}{" "}
      </Typography>
      <Box mt={0.3}>
      ...
      <Box sx={{width: { xs: "100%", sm: "90%",  md: "60%"}}} margin='auto'>
        <Typography fontSize={h6} pt={1} textAlign='center'>
          {text}
        </Typography>
      </Box>
    </Box>
